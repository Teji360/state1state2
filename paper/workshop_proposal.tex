...
PyramidKV, MagicPIG, FrozenKV) reflects a field that is generating results faster
...
\textbf{FrozenKV}, a novel warmup-then-freeze approach that computes an
...
The key insight behind FrozenKV is that autoregressive decode tokens are
...
\begin{itemize}[nosep,leftmargin=1.5em]
  \item \textbf{Per-token decode latency.}  After a shared warmup of 256 tokens,
    FrozenKV processes each new decode token in $0.17$--$0.18$\,ms (rank $8$--$32$),
  ...
  \item \textbf{Reconstruction error (real activations).}  OjaKV's relative
    ...
    zero matrix---direct evidence of stale-basis collapse.  FrozenKV's error
    is $0.27$--$0.44$ over the same range, $3$--$4.5\times$ lower.  At long
    context ($T{=}32$K, rank $16$) FrozenKV achieves $0.55$ versus OjaKV's $0.63$.
  \item \textbf{Memory at scale.}  At $T{=}128$K tokens the full float32 KV cache
    requires $9.44$\,GB; a FrozenKV pyramid basis compresses this to $2.21$\,GB
    ($4.3\times$) while beating OjaKV on reconstruction error at every long-context
    length tested ($1$K--$32$K).
  \item \textbf{End-task perplexity (GPT-2).}  FrozenKV (average rank $15$
  ...
  \item \textbf{Long-context perplexity (TinyLlama-1.1B, WikiText-2).}
    ...
    FrozenKV perplexity ranges from $36$ to $658$ while OjaKV degrades to
...
work introduces \textbf{FrozenKV}, a streaming KV compression method that computes
