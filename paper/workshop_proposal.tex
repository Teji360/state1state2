...
Recent arXiv activity (OjaKV~\cite{ojakov2025}, SVDq~\cite{svdq2025},
xKV~\cite{xkv2025}, KVTC, KQ-SVD, StreamingLLM~\cite{streamingllm2023},
SnapKV, PyramidKV, MagicPIG) reflects a field that is generating results
faster than the community can synthesize them.
...
\begin{itemize}[nosep,leftmargin=1.5em]
  \item \textbf{Per-token decode latency.}  After a shared warmup of 256 tokens,
    ...
    versus $0.46$--$0.55$\,ms for OjaKV---a $2.5$--$3.1\times$ speedup that is
  ...
  \item \textbf{Reconstruction error (real activations).}  OjaKV's relative
    Frobenius error is $\approx 1.2$ at every rank tested (rank $8$--$32$,
    $T{=}512$\,tokens of decode), meaning its reconstruction is about as bad as
    the zero matrix---direct evidence of stale-basis collapse.  TKV's error
    is $0.27$--$0.44$ over the same range, $2.7$--$4.4\times$ lower.  A separate
    sweep on \emph{synthetic} long-context data (not real activations) shows the
    same pattern holding out to $T{=}32$K: TKV $0.55$ versus OjaKV's $0.63$ at
    rank~$16$.
  \item \textbf{Memory at scale.}  At $T{=}128$K tokens the full float32 KV cache
    ...
    length tested ($1$K--$32$K, synthetic data).
  \item \textbf{End-task perplexity (GPT-2).}  TKV (average rank $15$
    ...
    $0.69$)---on this 117M-parameter model the frozen basis acts as a mild
    regularizer and \emph{improves} held-out perplexity.  Follow-up work on a
    7B-parameter model suggests this specific improvement does not generalize:
    there, TKV lands $3.6\%$ \emph{above} exact rather than below it, though
    still far closer to exact than OjaKV.  OjaKV under the same pyramid schedule
    achieves $756.2$ (ratio $5.83\times$ worse than exact at this scale, rising
    to $60\times$ worse on the 7B model), confirming that stale-basis error
    compounds into catastrophic language-model degradation that worsens with
    scale.
  \item \textbf{Long-context perplexity (TinyLlama-1.1B, WikiText-2).}
    ...
    \textbf{TKV+Sink} (full-precision sink and recent window, with the middle
    segment TKV-compressed) achieves perplexity ranging from $36$ to $658$
    while OjaKV degrades to $932$--$1795$, consistent with the stale-basis
    collapse observed above.  This sink+TKV composition is a distinct
    configuration from the plain-TKV results above; a full evaluation of it is
    ongoing work.
\end{itemize}
...
an EYM-optimal low-rank basis during prefill (via Brand's rank-1 SVD
update~\cite{brand2006} and Halko randomized range finding~\cite{halko2011})
and freezes the basis for the autoregressive decode
phase---exploiting the structure of conditional generation to achieve $2.5$--$3.1\times$
lower per-token latency than OjaKV with $2.7$--$4.4\times$ lower reconstruction error.
